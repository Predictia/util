...
/**
 * <p>
 * Utility class to guess the encoding of a given file or byte array. The guess
 * is unfortunately not 100% sure. Especially for 8-bit charsets. It's not
 * possible to know which 8-bit charset is used. Except through statistical
 * analysis.
 * </p>
 * 
 * <p>
 ...
 * </p>
 * 
 * This class is a heavily modified version of the original written by Guillaume
 * LAFORGE: com.glaforge.i18n.io.CharsetToolkit
 * 
 * taken from http://glaforge.free.fr/wiki/index.php?wiki=GuessEncoding
 * 
...
public class CharsetDetection {
	...
	public static Charset detectEncoding(InputStream is) throws IOException {
		return detectEncoding(IOUtils.toByteArray(is));
	}
	...
	public static Charset detectEncoding(byte[] data) throws IOException {
	 ...
	 * @param file
	 * @param bufferLength
	 *            number of bytes to read in for the detection. 8192 is a
	 *            reasonable value
	 * @return
	 * @throws IOException
	 */
	public static Charset detectEncoding(File file, int bufferLength)
			throws IOException {
	...
	private Charset detectCharacterEncoding8bit() {
		...
		// the number of "bad" chars for the best guess. A better guess will
		// have
		...
		log.debug("8bit Encoding guessed: " + bestEncoding + " with "
				+ leastSuspicousChars + " rare characters");
		return bestEncoding;
	}
	...
	private Charset detectUtf16() {
		...
		// first try to see if we got a little or big endian, i.e. lots of zeros
		// as the first byte or second byte if we deal
		...
		// a UTF16 encoding with many latin characters would have either lots of
		// even or uneven bytes as zero - but not both
		int min = (buffer.length / 10);
		if ((zerosBE > min || zerosLE > min)
				&& Math.abs(zerosBE - zerosLE) > min) {
			if (zerosBE > zerosLE) {
	...
	private long testLatin1() {
		...
		try {
			...
			// if that worked without a problem try to count suspicous
			// characters which are rarely used in our texts
			int length = buffer.length;
			int i = 0;
			while (i < length) {
				byte b = buffer[i];
				i++;
				// range 7f-9f undefined, see
	...
	private long testWindows1252() {
		...
		try {
			...
			while (i < length) {
				...
				i++;
				// 5 undefined chars
				if (b == (byte) 0x81 || b == (byte) 0x8d || b == (byte) 0x8f
						|| b == (byte) 0x90 || b == (byte) 0x9d) {
					suspicous += UNDEFINED_PENALTY;
				}
			}
		} catch (CharacterCodingException e) {
	 ...
	 * @throws IOException
	 */
	private static ByteBuffer readByteBuffer(File file) throws IOException {
